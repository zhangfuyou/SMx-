...
void show_bignum(BIGNUM *bn, int point_byte_length) {
	...
	char *to = BN_bn2hex(bn);	//取bn的十六进制值转换为字符串形式，返回指向此字符串的指针
...
BYTE *KDF(BYTE *str1, int klen, int strlen1) {
	...
	for (i = 0; i < group_number; i++)
	{
		...
		SM3_hash((BYTE *)str1, strlen1 + 4, (BYTE *)&H[i * HASH_BYTE_LENGTH]);
...
int sm2_hex2bin(BYTE *hex_string, BYTE *bin_string, int point_byte_length) {
	...
	int ret;
	b = BN_new();
	BN_hex2bn(&b, (char *)hex_string);		//赋16进制值hex_string到*b中,即将hex_string字符串转换为BIGNUM
...
//将bn转换为二进制，存于bin_string中，并返回字符串长度
//bin_string高位填0，低位存bn二进制。大端模式
int sm2_bn2bin(BIGNUM *bn, BYTE *bin_string, int point_byte_length) {
	...
	if (len > 0) {
		memset(bin_string, 0, len);			//len是bin_string的填充长度
	}

	ret = BN_bn2bin(bn, &bin_string[len]);			//取bn为二进制到&bin_string[len]中，返回字符串长度
	return ret;
}