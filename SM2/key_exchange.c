...
int dh_step1(sm2_dh_st *dh_data, BYTE *dh_d, BYTE *dh_r, ec_param *ecp) {
	...
	BUFFER_APPEND_STRING(Z_A.buffer, Z_A.possition, dh_data->ENTL, dh_data->ID);
	...
	DEFINE_SHOW_STRING(Z_A.buffer, Z_A.possition);
	
	SM3_hash(Z_A.buffer, Z_A.possition, Z_A.hash);

	memcpy_s(dh_data->Z, HASH_BYTE_LENGTH, Z_A.hash, HASH_BYTE_LENGTH);
...
/***************************************
*sm2_dh_st *dh_data_A	用户A的数据结构
*sm2_dh_st *dh_data_B	用户B的数据结构
*ec_param *ecp			曲线参数
*int order				用户顺序
***************************************/
int dh_step2(sm2_dh_st *dh_data_A, sm2_dh_st *dh_data_B, ec_param *ecp, int order) {
	...
	memcpy_s(dh_data_B->K, 256, K, dh_data_B->klen_bit / 8);
	OPENSSL_free(K);

	memset(&hash1, 0, sizeof(hash1));
	BUFFER_APPEND_BIGNUM(hash1.buffer, hash1.possition, ecp->point_byte_length, point_2->x);
	if (ORDER_A_B == order) {
		BUFFER_APPEND_STRING(hash1.buffer, hash1.possition, HASH_BYTE_LENGTH, dh_data_A->Z);
		BUFFER_APPEND_STRING(hash1.buffer, hash1.possition, HASH_BYTE_LENGTH, dh_data_B->Z);
		BUFFER_APPEND_BIGNUM(hash1.buffer, hash1.possition, ecp->point_byte_length, x_1);
		BUFFER_APPEND_BIGNUM(hash1.buffer, hash1.possition, ecp->point_byte_length, y_1);
		BUFFER_APPEND_BIGNUM(hash1.buffer, hash1.possition, ecp->point_byte_length, x_2);
		BUFFER_APPEND_BIGNUM(hash1.buffer, hash1.possition, ecp->point_byte_length, y_2);
	}
	else {
		BUFFER_APPEND_STRING(hash1.buffer, hash1.possition, HASH_BYTE_LENGTH, dh_data_B->Z);
		BUFFER_APPEND_STRING(hash1.buffer, hash1.possition, HASH_BYTE_LENGTH, dh_data_A->Z);
	...
	SM3_hash(hash1.buffer, hash1.possition, hash1.hash);
	DEFINE_SHOW_STRING(hash1.buffer, hash1.possition);

	//计算S
	memset(&dh_data_B->hash_tmp_data, 0, sizeof(dh_data_B->hash_tmp_data));
	dh_data_B->hash_tmp_data.possition = 1;
	BUFFER_APPEND_BIGNUM(dh_data_B->hash_tmp_data.buffer, dh_data_B->hash_tmp_data.possition,ecp->point_byte_length, point_2->y);
	BUFFER_APPEND_STRING(dh_data_B->hash_tmp_data.buffer, dh_data_B->hash_tmp_data.possition,HASH_BYTE_LENGTH, hash1.hash);
...
void key_exchange(char **sm2_param, int type, int point_bit_length) {
	...
	dh_step1(&dh_A, (BYTE *)sm2_param_dh_d_A[ecp->type], (BYTE *)sm2_param_dh_r_A[ecp->type], ecp);
	dh_step1(&dh_B, (BYTE *)sm2_param_dh_d_B[ecp->type], (BYTE *)sm2_param_dh_r_B[ecp->type], ecp);

	dh_step2(&dh_A, &dh_B, ecp, ORDER_A_B);		//K_B, S_B
	dh_B.hash_tmp_data.buffer[0] = 0x02;

	SM3_hash(dh_B.hash_tmp_data.buffer, dh_B.hash_tmp_data.possition, dh_B.hash_tmp_data.hash);
	DEFINE_SHOW_STRING(dh_B.hash_tmp_data.buffer, dh_B.hash_tmp_data.possition);
	memcpy_s(dh_hash.S_B, HASH_BYTE_LENGTH, dh_B.hash_tmp_data.hash, HASH_BYTE_LENGTH);


	dh_step2(&dh_B, &dh_A, ecp, ORDER_B_A);  //K_A, S_1
	dh_A.hash_tmp_data.buffer[0] = 0x02;
	SM3_hash(dh_A.hash_tmp_data.buffer, dh_A.hash_tmp_data.possition, dh_A.hash_tmp_data.hash);
	DEFINE_SHOW_STRING(dh_A.hash_tmp_data.buffer, dh_B.hash_tmp_data.possition);
	memcpy_s(dh_hash.S_1, HASH_BYTE_LENGTH, dh_A.hash_tmp_data.hash, HASH_BYTE_LENGTH);

	dh_A.hash_tmp_data.buffer[0] = 0x03; //S_A
	SM3_hash(dh_A.hash_tmp_data.buffer, dh_A.hash_tmp_data.possition, dh_A.hash_tmp_data.hash);
	memcpy_s(dh_hash.S_A, HASH_BYTE_LENGTH, dh_A.hash_tmp_data.hash, HASH_BYTE_LENGTH);

	dh_B.hash_tmp_data.buffer[0] = 0x03;  //S_2
	SM3_hash(dh_B.hash_tmp_data.buffer, dh_B.hash_tmp_data.possition, dh_B.hash_tmp_data.hash);
	memcpy_s(dh_hash.S_2, HASH_BYTE_LENGTH, dh_B.hash_tmp_data.hash, HASH_BYTE_LENGTH);
