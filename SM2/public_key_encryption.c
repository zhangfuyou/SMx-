...
int sm2_encrypt(ec_param *ecp, message_st *message_data)
{
	...
	BIGNUM *P_y = BN_new();
	BIGNUM *k = BN_new();
	xy_ecpoint *P = xy_ecpoint_new(ecp);
	xy_ecpoint *xy1 = xy_ecpoint_new(ecp);
	xy_ecpoint *xy2 = xy_ecpoint_new(ecp);
	int pos1;
	BYTE *t;
	int i;
	sm2_hash local_C_3;
	...
	BUFFER_APPEND_BIGNUM(local_C_3.buffer, local_C_3.possition, ecp->point_byte_length, xy2->x);
	BUFFER_APPEND_STRING(local_C_3.buffer, local_C_3.possition, message_data->message_byte_length, message_data->message);
	BUFFER_APPEND_BIGNUM(local_C_3.buffer, local_C_3.possition, ecp->point_byte_length, xy2->y);

	DEFINE_SHOW_STRING(local_C_3.buffer, local_C_3.possition);

	SM3_hash((BYTE *)local_C_3.buffer, local_C_3.possition, local_C_3.hash);
	memcpy_s(message_data->C_3,1024, (char *)local_C_3.hash, HASH_BYTE_LENGTH);

	DEFINE_SHOW_STRING(message_data->C_3, HASH_BYTE_LENGTH);
	...
	printf("encrypt: \n");
	DEFINE_SHOW_STRING(message_data->C, pos1);
...
int sm2_decrypt(ec_param *ecp, message_st *message_data)
{
	...
	DEFINE_SHOW_STRING(message_data->decrypt, message_data->message_byte_length);
